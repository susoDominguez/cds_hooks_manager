...
const {
  ...
  getOutcomeList, applyActions
} = require("./data-processing-modules/data-processing-module");
const { getModel } = require("../database_modules/dbConnection_Mongoose");
 ...
 */
  fetchParams : async function (req, res, next) {
    ...
    if (!Model)
      throw new ErrorHandler(
        500, "FetchParams function: Model collection name is undefined or there is a typo as it could not be found"
      );
    ...
    for await (const doc of Model.find().lean()) {
          ...
          if (paramKey === undefined)
            throw new ErrorHandler(500, `Funct FetchParams: Parameter ${paramName} may be missing from template in MongoDB`);
        ...
        default:
          ...
          if (!Array.isArray(cigList))
            throw ErrorHandler(500,
              "Funct fetchParams: parameter " + cigInvolved + " is not an Array as expected."
            );
  ...
  getCdsServices : async function (req, res, next) {
    //cds services array
    res.locals.cdsServices = [];
    //loop over documents
    for await (const service of Model.find().lean()) {
      //add cds service to array
      res.locals.cdsServices.push(service);
    }
    //return array
    next();
  }
